...
public class LivreService {
    ...
    public boolean addLivre(Livre livre) {
        if (!livre.getTitre().isEmpty() && !livre.getAuteur().isEmpty()) {
            String sql = "INSERT INTO livres (titre, auteur, isbn, quantite_totale, quantite_disponible, statut) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public boolean updateLivre(Livre livre) {
        int row = 0;
        String sql = "UPDATE livres SET titre = ?, auteur = ?, isbn = ?, quantite_totale = ?, quantite_disponible = ?, statut = ? WHERE id_livre = ?";
